...
export default function KahootClone() {
	...
	const checkAllPlayersAnswered = useCallback(async () => {
		...
		try {
			...
			if (uniquePlayerIds.size >= players.length) {
				...
				setTimeout(() => {
					nextQuestion();
				}, 2000); // Show results for 3 seconds before moving on
			}
		} catch (error) {
	...
	useEffect(() => {
		...
		const fetchAnswers = async () => {
			const { data, error } = await supabase
				.from("answers")
				.select("id, player_id, question_id, selected_answer")
			...
			.on(
				...
				(payload) => {
					...
					setQuestionAnswers((prev) => {
						if (prev.some((a) => a.player_id === answer.player_id)) return prev;
						return [...prev, answer];
					});
				},
			)
		...
		// Fallback: answer submit also updates player scores
		const playersChannel = gameId
			? supabase
					.channel(`answers-fallback-players-${questionId}`)
					.on(
						"postgres_changes",
						{
							event: "*",
				...
				<Card className="w-full max-w-2xl">
					<CardHeader className="text-center">
						<CardTitle className="text-3xl font-bold">
							Game Room: {gameState.id} {isHost ? "(Host)" : "(Player)"}
						</CardTitle>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								{currentQuestion.options.map((option, index) => {
									let buttonVariant:
										...
										| "destructive"
										| "secondary" = "outline";
									let buttonClass = "p-6 text-lg h-auto w-full";
									...
									const voters = isHost
										? questionAnswers
												.filter((a) => a.selected_answer === index)
												.map((a) => ({
													id: a.player_id,
													name:
														players.find((p) => p.id === a.player_id)?.name ??
														"Unknown",
												}))
										: [];
